	...
	*/
	...
	flickrupcomingevent.prototype = {
		...
		init: function() {
			...
			if(unsafeWindow.page_photo_id) {
				...
				if(!this.upcoming_token) {
					...
					img.align="left";
					img.alt=this.localiser.localiser("script_title");
		...
		displayEventInfo: function(div) {
			...
			li.className="Stats";
			li.innerHTML = '<b>'+this.localiser.localise('event')+':</b> <a href="http://www.upcoming.org/event/'+event.id+'">'+event.name+'</a> ';
			if(this.canTag) {
			...
			var li = ul.appendChild(document.createElement('li'));
			li.className="Stats";
			li.innerHTML = '<b>'+this.localiser.localise('metro')+':</b> '+this.localiser.localise('searching')+'...';
			...
			var li = ul.appendChild(document.createElement('li'));
			li.className="Stats";
			li.innerHTML = '<b>'+this.localiser.localise('venue')+':</b> '+this.localiser.localise('searching')+'...';
			...
			var li = ul.appendChild(document.createElement('li'));
			li.className="Stats";
			li.innerHTML = this.localiser.localise('see_other',{
		...
		countryInfoReceived: function(rsp,li,metro,state) {
			...
			li.innerHTML = '<b>'+this.localiser.localise('metro')+':</b> <a href="http://upcoming.org/metro/'+country.@code+'">'+country.@name+'</a> > <a href="http://upcoming.org/metro/'+country.@code+'/'+state.@code+'">'+state.@name+'</a> > <a href="http://upcoming.org/metro/'+country.@code+'/'+state.@code+'/'+metro.@code+'">'+metro.@name+'</a> ';
			if(this.canTag) {
				var a = li.appendChild(document.createElement('a'));
				a.setAttribute('style',"text-decoration: none; color: rgb(201, 201, 201);");
				a.href="javascript:;";
