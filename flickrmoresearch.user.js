...
(function () {
	...
	win.FlickrMoreSearch.prototype = {
		...
		initHeaderSearch: function() {
			...
			method.type = 'hidden';
			method.name = 'm';
			searchbox.appendChild(method);
			var thisthing = null;
			
			
			//insert the select to choose from groups/people/photo search
			var searchbutton = document.getElementById('headersearchbutton2');
			var select = searchbox.insertBefore(document.createElement('select'),searchbutton);
			select.innerHTML = '<option value="photos" selected="true">photos</option>\
...
<option value="people">people</option>';
			select.addEventListener('change',function(event) {
				if(select.value == 'photos') {
					...
					if(thisthing && thisthing.checked && searchbox.action == 'http://www.flickr.com/search/groups/') {
						method.value="pool";
					} else 
						searchbox.action = 'http://www.flickr.com/search/';
				} else if(select.value == 'discussions') {
					if(span) span.style.display = 'inline';
					searchbox.action = 'http://www.flickr.com/search/groups/';
					method.value="discuss";
				} else {
					method.value="";
					if(span) span.style.display = 'none';
					searchbox.action = 'http://www.flickr.com/search/'+select.options[select.selectedIndex].value+'/';
				}
			},true);


			var span = null;
			if(matches = /^http:\/\/(www\.)?flickr.com\/(photos|people|groups)(\/[^\/]+\/?|_members.gne\?id=([^&]+))/.exec(loc)) {

				// inserts the checkbox to search the curent group/user
				searchbox.appendChild(document.createElement('br'));
				span = document.createElement('span');
				thisthing = span.appendChild(document.createElement('input'));
				thisthing.name = 'w';
				thisthing.value = this.userID;
				thisthing.type = 'checkbox';
				thisthing.id = 'thisthing';
				thisthing.disabled = true;
				var thisthinglabel = span.appendChild(document.createElement('label'));
				thisthinglabel.htmlFor = "thisthing";
				thisthinglabel.innerHTML = "search this ";
				searchbox.appendChild(span);

				if(matches[2] == 'photos' || matches[2] == 'people') {
					if(matches[3].indexOf('/friends') < 0) {
						thisthinglabel.innerHTML += ' user.';
						var self = this;
						var listener = {
							flickr_urls_lookupUser_onLoad: function(success, responseXML, responseText, params){
								if(success) {
									var users = responseXML.documentElement.getElementsByTagName('user');
									thisthing.value = users[0].getAttribute('id');
									if(thisthing.value == self.userID) {
										thisthinglabel.innerHTML = "Search your photos.";
									}
									thisthing.disabled = false;
								}
							}
						};
						
						unsafeWindow.F.API.callMethod('flickr.urls.lookupUser', {url:matches[0]}, listener);
					} else {
						thisthing.value = this.userID;
						thisthinglabel.innerHTML = "Search your photos.";
						thisthing.disabled = false;
					}
				} else { //if(matches[2] == 'groups') {
						thisthinglabel.innerHTML += ' group.';
						thisthing.addEventListener('click',function(){
													   if(method.value == 'pool') method.value = '';
													   else method.value = 'pool';
												   }, true);
						thisthing.disabled = false;
						
						/*var title = document.evaluate("/html/body/div[@id='Main']/table[@id='SubNav']/tbody/tr/td[2]/h1|/html/body/div[@id='Main']/h1[@id='Tertiary']/a[2]",
						  document.body, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
						  ).singleNodeValue;	
						  if(title) {
						  var titlehidden =span.appendChild(document.createElement('input'));
						  titlehidden.type = 'hidden';
						  titlehidden.name = 'group_title';
						  titlehidden.value=title.textContent.replace(/^\s+|\s+$/g,'');
						  }*/
						if(matches[4]) {
							thisthing.value = matches[4];
							thisthing.form.action = "http://www.flickr.com/search/groups/";
						} else {
							var listener = {
								flickr_urls_lookupGroup_onLoad: function(success, responseXML, responseText, params){
									if(success) {
										...
										thisthing.value = groups[0].getAttribute('id');
										thisthing.form.action = "http://www.flickr.com/search/groups/";
									}
								}
							};
							
							unsafeWindow.F.API.callMethod('flickr.urls.lookupGroup', {url:matches[0]}, listener);
						}
				}
			}		
		},
