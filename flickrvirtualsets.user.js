...
// v2.1 08/27/06        Mortimer? (Pierre Andrews) contributes "Relevant Photos"
//
...
if( re.test(document.location) || re3.test(document.location) ) {
    var tags = '';
    for(tag in w.global_photos[w.page_photo_id].tags_rawA) {
	tag_value = w.global_photos[w.page_photo_id].tags_rawA[tag];
	if(tag_value.indexOf('geo:')<0) tags += ','+ tag_value;
    }
    tags = tags.substr(1);
    GM_log('Tags: ' + tags);
} else tags = '';
...
var relevantphotos = {
    ...
    slide: '',
    browse: 'http://flickr.com/search/?q='+tags+'&m=tags&z=t',
    paged: true,
    api_paged: false,
    perpage: 24,
    addself: true,
...
vsets['vset13'] = { enabled: function() { return GM_getValue('vset13',false); }, config: relevantphotos };
...
function Photostream(config) {
	...
	if( config.addself && index==-1 ) index=0;
	if( index<0 ) {
...
function newPStreams() {
    ...
    if(vsets['vset4'].enabled() && !vsets['vset4'].stream ) vsets['vset4'].stream = new Photostream(interestingness);
    if(vsets['vset13'].enabled() && !vsets['vset13'].stream ) vsets['vset13'].stream = new Photostream(relevantphotos);
...
//	vsets['vset4'].stream = new VirtualSet(interestingness);
    ...
    if(vsets['vset5'].enabled() && !vsets['vset5'].stream ) vsets['vset5'].stream = new VirtualSet(geotagged);
    if(vsets['vset6'].enabled() && !vsets['vset6'].stream ) vsets['vset6'].stream = new VirtualSet(friends);
    if(vsets['vset13'].enabled() && !vsets['vset13'].stream ) vsets['vset13'].stream = new VirtualSet(relevantphotos);
}
